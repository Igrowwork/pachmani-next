...
export default function WishList() {
  ...
  const {error,loading,product} = useSelector((state: RootState) =>state.wishlist)
  useEffect(()=> {
    dispatch(getWishlistCart())
  } , [])
  

  // const isFetch = async (id:string) => {
  //   try{
  ...
  // const isFetch = async () => {
  //   try{
  //     const res = await api.post("product/wishlist")
  //     // console.log(res.data?.wishlist , "12345")
  //     setIsApi(res.data?.wishlist)
  //   }
  //   catch(err){
          ...
          {product?.map((ele, i) => (
            <div className="rounded-2xl shadow-[2px_2px_20px_0px_rgba(0,0,0,0.10)] my-3 overflow-auto hover:scale-105 transition-all duration-300 ease-in-out">
              <div className="relative md:h-52 h-44 w-full">
                ...
                >
                  {ele.isLike ? <Icons.notLike /> :  <Icons.like />}
                </div>
              </div>
