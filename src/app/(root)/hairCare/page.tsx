...
const hairCare = () => {
  ...
  useEffect(()=> {
    const apiFetch = async () => {
      try{
        const res = await axios.get(`${process.env.NEXT_PUBLIC_URL}/api/product?category=hairCare`)
        // console.log(res?.data?.products,"qwert")
        setIsData(res?.data?.products)
      }
      catch(e){
        console.log(e, "hair care");
      }
    }
    apiFetch();
  },[])
        ...
        <div className='grid xl:grid-cols-4 md:grid-cols-2 gap-5 my-10 p-2'>
          {
            ...
            <div className='grid p-2 gap-1'>
              <h3 className='text-xs text-primaryMain font-medium capitalize'>{ele?.category}</h3>
                <h2 className='text-base font-medium mt-1'>oil</h2>
                <div className='md:text-sm text-xs text-[#313131] '>{ele?.description}</div>
                <div className='my-2 '>
