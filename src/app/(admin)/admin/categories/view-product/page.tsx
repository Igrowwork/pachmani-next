    ...
    <div>
      ...
      { arr?.map((ele,i) => (
                 ...
                 <div className='relative md:h-52 h-44 w-full'>
                     ...
                     <div className="absolute top-0 right-0 p-5 cursor-pointer">{<Icons.notLike /> }</div>
                 </div>
                 <div className='grid p-2 gap-1'>
                   <h3 className='text-xs text-primaryMain font-medium capitalize'>{ele?.title}</h3>
                     <h2 className='text-base font-medium mt-1'>{ele?.name}</h2>
                     <div className='text-xs text-[#313131] '>{ele?.data} </div>
                     <div className='my-2 '>
