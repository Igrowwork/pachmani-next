        ...
        arr2?.map((ele, i) => (
            <div key={i}>
                <div className='relative h-48 w-full rounded-xl overflow-hidden'>
                    <Image src={'/Assests/Images/HairImage/05.png'} alt='No Preview' fill className='object-cover' />
                    <div className='absolute top-0 right-0 p-3'>
                        <CiMenuKebab className='cursor-pointer bg-white rounded-full' onClick={() => setIsVal1(!isVal1)} />
                        {isVal1 &&
                            <div className="absolute top-5 right-4 shadow-md m-2 p-1 grid bg-white rounded-sm w-20 h-20">
                                <h1 className="font-semibold">Actions</h1>
                                <div className="grid text-secondary-main text-xs gap-1">
                                    <div className="flex gap-2 w-full cursor-pointer hover:text-primary-main" onClick={() => { setCurrentItem(ele); setIsPopupVisible(true); setIsVal1(false); }}><FaRegEdit /> Change</div>
                                    <div className="flex gap-2 w-full cursor-pointer hover:text-primary-main" onClick={() => setIsVal1(false)}><MdDelete /> Remove</div>
                                </div>
                            </div>
                        }
                    </div>
